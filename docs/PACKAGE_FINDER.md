...
| GDScript high fan-in/fan-out and possible unused script review | `python -m pip install godot-gdscript-architecture-guard` | `godot-architecture-guard . --config architecture-guard.toml --format markdown --fail-on none` |
...
| Godot workflow-grouped release dashboard with typed highlights and previous-run trends | `python -m pip install godot-release-dashboard-kit` | `godot-release-dashboard build reports/current --previous-reports-dir reports/previous --output reports/dashboard.html` |
...
- Start with `godot-release-dashboard-kit` when a release or pull request has
  several reports, typed summary highlights, and screenshots that should be
  reviewed in one static page.
