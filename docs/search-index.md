...
For already-built artifacts, use `godot-export-doctor inspect-folder`,
`godot-export-doctor inspect-files`, or `godot-export-doctor pck` to review
exported folder contents, generated file lists or pack manifests, extension
summaries, file sizes, optional hashes, and development or private-looking files
before sharing build artifacts.
...
Use `godot-export-doctor inspect-files` or the shorter `godot-export-doctor pck`
when your build pipeline emits a text or JSON export file list. Direct `.pck`
binary parsing is not part of this check; inspect a generated manifest or file
list instead.
...
can generate a baseline fixture from a schema, validate existing fixtures, and
...
retry grouping, JUnit XML summaries, and HTML summaries. It is designed for
JSON output from scenario runners, smoke tests, custom harnesses, and JUnit XML
from existing Godot test runners.
