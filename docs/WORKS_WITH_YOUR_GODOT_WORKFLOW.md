...
- `godot-release-dashboard-kit` turns existing reports and screenshots into a
  workflow-filtered static review page, with optional previous-run readiness
  trends.
...
|---|---:|---|
| `godot-production-doctor` | No | Installs the `godot-project-doctor` command, recommends package installs, runs and summarizes toolkit checks, and can write profile-based Markdown setup plans. It may inspect a Godot project directory, but the CLI itself does not launch Godot. |
...
| `godot-release-dashboard-kit` | No | Builds workflow-filtered static HTML dashboards from existing toolkit reports and visual artifacts, with optional previous-run readiness trends and comparison cards. |
