...
1. Command discovery was implicit. Users had to read prose docs and infer commands, output formats, write behavior, and exit-code semantics.
2. Visual or generative tools had weaker JSON status output than the audit tools.
3. Interface guidance was split across READMEs and docs, without a shared tool manifest.

This pass adds a stable `tool-manifest.json` manifest to every tool, a root `tool-manifest.schema.json`, and `verify_tool_manifests.py` to check the manifests. It also adds JSON/status output for `pixel-space-assets` generation commands, JSON plan output for `godot-visual-smoke plan`, an umbrella runner, a GitHub Action wrapper, and neutral project metadata.

## Implemented Fixes

- Added `tool-manifest.json` to each tool folder.
- Added `tool-manifest.schema.json` at the suite root.
- Added `verify_tool_manifests.py` for local/CI validation of manifest presence and required fields.
...
- Updated relevant README examples for automation-friendly JSON usage.
...
| Tool | Standalone CLI | Machine Output | Human Output | Notes |
|---|---:|---:|---|---|
...
| `godot-visual-smoke-test-kit` | yes | JSON | PNG diff images | Needs project-owned Godot capture helper for full end-to-end use. Plan output is now easy for scripts to consume. |
...
| `pixel-space-asset-toolkit` | yes | JSON status/manifests | PNG gallery/previews | Suitable for repeatable asset-generation scripts. Later add parameter presets and an HTML gallery command. |
...
For the first release, publish as one umbrella repository so users can see the suite, CI, action, and maintenance model together. If packages later split into separate repositories, keep each folder's `tool-manifest.json` at the repository root.
