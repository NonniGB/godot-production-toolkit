...
- Added `AI_REVIEW_PACKAGE.md` and `oss-review-evidence.json`.
...
5. Publish packages and collect organic public usage evidence.

## Publication Notes

For the first release, publish as one umbrella repository so automated reviewers see the suite, CI, action, and maintainer evidence together. If packages later split into separate repositories, keep each folder's `agent-tool.json` at the repository root.
